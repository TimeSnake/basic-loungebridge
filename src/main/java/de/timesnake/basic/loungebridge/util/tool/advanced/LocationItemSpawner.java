...
public class LocationItemSpawner implements GameTool, StartableTool, StopableTool {
  ...
  public LocationItemSpawner(ExLocation location, TimeUnit unit, int delay, List<? extends ItemStack> items) {
    this(location, unit, delay, 0, items);
  }

  public LocationItemSpawner(ExLocation location, TimeUnit unit, int delay, int delayRange,
                             List<? extends ItemStack> items) {
    this(location, unit, delay, delayRange, () -> List.of(items.get(Server.getRandom().nextInt(items.size()))));
  }

  public LocationItemSpawner(ExLocation location, TimeUnit unit, int delay, int delayRange, Supplier<List<?
  ...
  @Override
  public void start() {
    this.logger.info("Starting item spawner '{}", this.location);
    ...
    this.task = Server.runTaskTimerSynchrony(() -> {
      ...
      if (delay <= 0) {
        this.itemSupplier.get().forEach(i -> location.getWorld().dropItem(this.location, i));
        this.delay = (this.delayRange > 0 ? Server.getRandom().nextInt(delayRange) : 0) + delayBase;
      }
  ...
  public void stop() {
    ...
    this.logger.info("Stopping item spawner '{}'", this.location);
  }
